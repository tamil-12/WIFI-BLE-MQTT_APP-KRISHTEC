...
class MqttConnectedPage extends StatefulWidget {
  final MqttServerClient client;
  final String topic;


  const MqttConnectedPage({Key? key, required this.client, required this.topic}) : super(key: key);


  @override
  _MqttConnectedPageState createState() => _MqttConnectedPageState();
}

class _MqttConnectedPageState extends State<MqttConnectedPage> {
  ...
  final _prefsKey = 'mqtt_widgets'; // Key for storing widgets in SharedPreferences
  ...
  void initState() {
    super.initState();
    _subscribeToTopic();
    _loadSavedItems(); // Load saved items when the widget initializes


  }
  void _subscribeToTopic() {
    // Subscribe to the MQTT topic
    widget.client.subscribe(widget.topic, MqttQos.exactlyOnce);
    ...
    widget.client.updates!.listen((List<MqttReceivedMessage<MqttMessage>> messages) {
      ...
      // Show dialog with received message
      showDialog(
        context: context,
  ...
  void _saveItems() async {
    ...
    List<String> itemsJson = _items.map((item) => json.encode(item.toJson())).toList();
    await prefs.setStringList('${_prefsKey}_${widget.topic}', itemsJson); // Save with topic prefix
  }

  void _loadSavedItems() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    List<String>? itemsJson = prefs.getStringList('${_prefsKey}_${widget.topic}'); // Load with topic prefix
    if (itemsJson != null) {
      List<ItemModel> savedItems = itemsJson.map((jsonString) => ItemModel.fromJson(json.decode(jsonString))).toList();
      setState(() {
  ...
  Future<void> _sendData(String action) async {
    ...
    widget.client.publishMessage(widget.topic, MqttQos.exactlyOnce, builder.payload!);
  }
  ...
  void _sendpin(int selectedPin, String selectedPinMode) {
    ...
    // Create a payload builder and add the JSON string
    final MqttClientPayloadBuilder builder = MqttClientPayloadBuilder();
    builder.addString(jsonData);

    // Publish the message using the client and topic passed to the widget
    widget.client.publishMessage(widget.topic, MqttQos.exactlyOnce, builder.payload!);
  }
    ...
    if (!_isDisconnected) {
      showMenu(
        ...
        if (value != null) {
          ...
          if (exists) {
            setState(() {
              _items.add(ItemModel(type: value,topic:widget.topic));
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              bool isPinAlreadyAssigned =
              _items.any((item) => item.selectedPin == selectedPin && item.type != 'Display');
...
class ItemModel {
  ...
  late double value;
  int? selectedPin;
  late String topic; // Added topic property
  ...
  ItemModel({
    ...
    this.value = 0,
    this.selectedPin,
    required this.topic, // Initialize topic
  });

  // Convert item model to JSON
  Map<String, dynamic> toJson() {
    return {
      ...
      'value': value,
      'selectedPin': selectedPin,
      'topic': topic, // Save topic
    };
  }

  // Factory method to create item model from JSON
...
//   }
//   Widget buildItem(ItemModel item, int index) {
//     double boxHeight = 320; // Initial height of the box
//     double boxWidth = double.infinity; // Initial width of the box
//
...
//   final Widget child;
//
//   const ResizableCard({Key? key, required this.child}) : super(key: key);
//
//   @override
